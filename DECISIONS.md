...
Increment 4.2a implements the mixed message/custom tree and pending codecs, idle custom append, mixed query surface, and explicit message narrowing for operation-owned identities. The Test pass found and the implementation corrected a caller-mutation race by capturing the detached custom payload synchronously before mutation-line admission. Fresh independent implementation review PASS; complete Harness 581/581, root check, and diff check PASS.
...
Implementation commit `89b694f95` activates the approved boundary. Codec and exact bounded hydration accept mixed pending writes while message-only queues remain strict; pending identities are checked against entries, usage rows, current logical reservations, and every other pending owner. Write cancellation updates total operation state before deleting only the selected pending register; abort retains writes unchanged and repeated abort stays write-free; finish rejects residual writes while its defensive operation inventory remains complete. Planner and façade admission behavior are unchanged. Focused Tier A/B/C passes 171/171, complete Harness passes 618/618, root check and diff check pass, and fresh independent implementation review reports PASS.

## D-052 — Admit active writes and place an exact FIFO prefix atomically

- Date: 2026-08-13
- Phase: 4
- Status: accepted after corrected independent design review
- References: D-047–D-051; `packages/agent/docs/harness-v3.md` §§1.4, 2.2, 3.3–3.5, 3.11–3.13, 4.1, 4.4, 4.6–4.7, 5.3, 9.1–9.3

### Options

1. Give RuntimeShell separate idle-append and active-admission capabilities and choose between them from its current attachment snapshot.
2. Keep one owner append capability and let StoredSession choose direct placement or pending admission from authoritative state inside the mutation line.
3. Send an expected attachment through the append capability and return `stale` for RuntimeShell to retry with a newly minted id.

### Choice

Option 2. One public façade call captures and validates caller input synchronously, then the owner capability mints exactly one UUIDv7 and decides under the StoredSession mutation line. An idle lane keeps `TX[entry, lane.leaf]`. Every active run phase, under either running or cancelled control, accepts `TX[pending.entry/{id}, total op.state with inbox.writes += id]`. The capability returns exactly `{status:"placed"|"pending", entryId, attachment}`; `placed` means the entry is materialized, its pending register is absent, and the leaf moved to it, while `pending` means its register is present, the entry is absent, and the write id was appended exactly once. RuntimeShell publishes the exact postcommit attachment constructed from authoritative precommit state and `CommitResult`, then returns the one id without a postcommit read. This avoids stale dispatch, duplicate id minting, and retry semantics.

The id is minted only after lifecycle admission and must be absent from the current logical-id inventory, committed entries, usage rows, `op.meta`, `op.state`, and `pending.entry`. A collision follows the existing generated-id contract: reject as storage failure, fault RuntimeShell, commit nothing, publish no attachment, and never remint. Admission, cancellation, abort, finish, and close retain their existing serialization and lifecycle precedence. Close that seals first rejects without minting or writing; an already admitted append completes before close.

### Private placement transition

StoredSession gains one package-private transition that accepts an operation id, the expected operation-state and leaf authorities, an exact non-empty FIFO-prefix id list, and exactly one `{entryId, projection:"projecting"|"unprojected"}` item per selected id. Both arrays must have equal nonzero length, contain no duplicate ids, and contain the same ids in the same order. A message must be classified projecting; a custom entry may use either classification. A malformed vector or a classification incompatible with its pending payload is an invalid private transition and rejects write-free; missing or malformed durable pending payload after current authority matches is corruption. The transition calls no projector and is not exported from the package root. The next increment can run projectors outside the mutation line and invoke this unchanged seam.

Inside the mutation line, placement reloads complete current authority and requires the same lane operation, operation-state sequence, leaf sequence/value, exact FIFO prefix, intact pending payloads, per-id classification, and no selected entry or usage materialization. Under running control only `checkpoint` with `skipInboxOnce !== true` and `failure_drain` are eligible; a skip-once checkpoint must perform its required generation pass before later writes can place. Under cancelled control, `checkpoint` regardless of skip-once, `failure_drain`, assistant `ready` or `retry_wait`, and a tools phase whose calls are all completed are eligible because they own no unsettled intended effect; assistant `effect_pending` and every tools phase containing a planned or effect-pending call must reconcile first. Changed owner, operation-state, leaf, or FIFO authority returns the fresh attachment as stale with zero writes. Matching authority with a missing or malformed pending payload or entry/usage overlap is durable corruption with zero writes. Valid placement commits exactly `TX[entries in FIFO parent order, pending.entry deletes in FIFO order, lane.leaf = final id, total op.state]`; all entries share the transaction timestamp and receive the corresponding storage-assigned write sequences. The returned attachment is constructed without a postcommit read.

Under running control, a batch containing a message or projecting custom entry enters `checkpoint{need_assistant(false), triggerEntryId:finalId, skipInboxOnce:true}` and resets overflow recovery. An all-unprojected custom batch preserves the complete prior phase and continuation. Under cancelled control, every batch preserves the complete prior phase regardless of projection. All cases remove only the selected prefix and preserve later admissions.

Cancellation, abort, later admission, and placement serialize into one of two complete histories. A prior state-changing commit makes an old placement authority stale and write-free; placement that commits first makes later cancellation report consumed and leaves later abort to retain the remaining writes. No planner action, projector/context execution, Storage/schema change, SQLite process test, structural-operation waiter, or `pi-ai` change belongs to 4.2d.
...
Ordinary state mutations and operation progress—including prompt, tree writes, queues, cancellation, and manual actions—serialize on the admission line and wait for a running idle reservation there. Abort uses reservation-aware interrupt admission instead: it waits an idle callback but must bypass a long-running provider, tool, or post-effect await so §4.6 can durably mark cancellation and pull the effect signal. Its durable mutation still linearizes on the StoredSession mutation line, and close tracks an admitted abort before closing the owner. SessionTree reads and `peekAction` use a separate tracked read admission that bypasses the line and reservation, so a callback may read the lane while holding its reservation; close seals and waits those reads. Async-local callback ownership makes every same-lane state mutation, abort, or close fail immediately with `RuntimeShellError("active", ...)`, while external mutations continue waiting for the reservation. While the lane remains durably active no pending reservation is installed, so later active-operation jobs may lawfully commit; the exact terminal publication installs the reservation synchronously before a queued post-terminal mutation can start a new operation.
